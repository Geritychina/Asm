...
int do_tests(int a) {
	...
	int p4;
	_asm {
		mov al, byte ptr p2; get the lowest byte from p2 and assign it to al
...
int main()
{
	...
	ptr2Int = &myInt; // set the pointer to point to myInt
	int& ref2int = myInt; // reference requires initialization. Another name for another memory cell

	cout << ptr2Int << endl; // the address 006FFB24
	cout << *ptr2Int << endl; // 3 Dereference. Operand of '*' must be a pointer
	cout << ref2int << endl; // 3
	cout << &ref2int << endl; // the reference point to the same memory cell 006FFB24 
	cout << &myInt << endl; // The address of the variable 006FFB24
	myInt = 12;
	cout << myInt << endl; // 12
	cout << ref2int << endl; // 12
